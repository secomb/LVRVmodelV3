//nrutil.cpp
//this version modified to include d4tensor
//TWS, December 2019
//modified to include d4tensor
//TWS, February 2024
...
/* allocate a float matrix with subscript range m[nrl..nrh][ncl..ncb] */
{
...
/* allocate a double matrix with subscript range m[nrl. .nrh] [ncl. .nch] */
{
	...
	if (!m) nrerror("alocation failure 1 in matrix()");
	m += NR_END;
	m -= nrl;
...
double ****d4tensor(long nrl, long nrh, long ncl, long nch, long ndl, long ndh, long nhl, long nhh)
/* allocate a double 4tensor with range t[nrl. .nrh] [ncl. .nch] [ndl. .ndh] [nhl. .nhh] */
{
	long i, j, k, nrow = nrh - nrl + 1, ncol = nch - ncl + 1, ndep = ndh - ndl + 1, nhei = nhh - nhl + 1;
	double ****t;
	...
	/* allocate pointers to pointers to rows and set pointers to pointers to them */
	t[nrl] = (double ***)malloc((size_t)((nrow*ncol + NR_END) * sizeof(double**)));
	...
	/* allocate rows and set pointers to them */
	t[nrl][ncl][ndl] = (double *)malloc((size_t)((nrow*ncol*ndep*nhei + NR_END) * sizeof(double)));
	...
	// arrays of pointers
	for (i = nrl + 1; i <= nrh; i++) {
		t[i] = t[i - 1] + ncol;	//1D: edge
		t[i][ncl] = t[i - 1][ncl] + ncol * ndep;	//2D: edge
		t[i][ncl][ndl] = t[i - 1][ncl][ndl] + ncol * ndep * nhei;	//3D: edge
	}
	for (i = nrl; i <= nrh; i++) for (j = ncl + 1; j <= nch; j++) {
		t[i][j] = t[i][j - 1] + ndep;	//2D: face, starts at nrl not nrl+1
		t[i][j][ndl] = t[i][j - 1][ndl] + ndep * nhei;	//3D: face, starts at nrl not nrl+1

	}
	for (i = nrl; i <= nrh; i++) for (j = ncl; j <= nch; j++) for (k = ndl + 1; k <= ndh; k++)
		t[i][j][k] = t[i][j][k - 1] + nhei;	//starts at ncl not ncl+1, nrl not nrl+1

	/* return pointer to pointers to pointers to rows */
	return t;
}
...
void free_d4tensor(double ****t, long nrl, long nrh, long ncl, long nch, long ndl, long ndh, long nhl, long nhh)
/* free a double d4tensor allocated by d4tensor() */
{
	free((FREE_ARG)(t[nrl][ncl][ndl] + nhl - NR_END));
	free((FREE_ARG)(t[nrl][ncl] + ndl - NR_END));
	free((FREE_ARG)(t[nrl] + ncl - NR_END));
	free((FREE_ARG)(t + nrl - NR_END));
}
